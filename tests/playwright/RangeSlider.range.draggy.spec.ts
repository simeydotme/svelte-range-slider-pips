...
test.describe('Range Tests', () => {
  ...
  test.describe('draggy=true', () => {
    test('range bar should be draggable and maintain handle spacing', async ({ page }) => {
      ...
      await expect(handles.nth(1)).toHaveCSS('left', '300px');
    });
    ...
    test('handles should start to converge when dragged to range limits', async ({ page }) => {
      ...
      await expect(handles.nth(0)).toHaveCSS('left', '850px');
      await expect(handles.nth(1)).toHaveCSS('left', '1000px');
      // reset the handles
      ...
      await expect(handles.nth(0)).toHaveCSS('left', '950px');
      await expect(handles.nth(1)).toHaveCSS('left', '1000px');
      ...
      await expect(handles.nth(1)).toHaveCSS('left', '0px');
    });
    ...
    test('keyboard controls should work normally with draggy enabled', async ({ page }) => {
      ...
      await expect(handles.nth(0)).toHaveCSS('left', '410px');
      await expect(handles.nth(1)).toHaveCSS('left', '600px');
      ...
      await expect(handles.nth(0)).toHaveCSS('left', '500px');
      await expect(handles.nth(1)).toHaveCSS('left', '600px');
    });
