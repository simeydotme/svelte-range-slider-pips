...
<script  lang="ts">
  ...
  const formatter = (v:number) => { return num.format(v); };

  let day = [3];
  const days = [ "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Happy Days" ];
  const dayFormat: Formatter = (v) => days[v];
  const dayFormatCn = (v:number) => { if (v === 6) { return "星期日"; }; return "星期" + numzh.format(v + 1); };
  ...
  let big = [500,1000];
  let hramp: Formatter = (v,i,p) => { return ((v) * (((p/100)*(p/100)) )).toFixed(0) + " (" + p + "%)"; };
  let ramp: Formatter = (v,i,p) => { return ((v) * (((p/100)*(p/100)) )).toFixed(0); };
  ...
  <div class="content" style="--range-handle-focus: {color}; --range-handle: {lightColor}">
    ...
    <table> 
      ...
      <tr>
        ...
        <td><code>0.1</code></td>
        <td><RangeSlider id="test-id3" springValues={{ stiffness: .75, damping: 1 }} bind:values={values} {reversed} {hoverable} {disabled} /></td>
      </tr>
      <tr>
        <td><code>0.1</code></td>
        <td><code>0.85</code></td>
        <td><RangeSlider id="test-id2" springValues={{ stiffness: .1, damping: .857 }} bind:values={values} {reversed} {hoverable} {disabled} /></td>
      </tr>
      <tr>
        <td><code>0.05</code></td>
        <td><code>0.05</code></td>
        <td><RangeSlider id="test-id" springValues={{ stiffness: .05, damping: 0.05 }} bind:values={values} {reversed} {hoverable} {disabled} /></td>
      </tr>
    </table>
   ...
   {reversed} {hoverable} {disabled} />
    ...
    <button on:click={()=>{ zeromin = 10; zeromax = 30; zero = [3,70]; }}>increase min/max</button> - {zero} 
    
    <h4>push & pop values</h4>
    <RangeSlider bind:values={values} float pips all="label" {reversed} {hoverable} {disabled} ariaLabels="x" />
   ...
   {reversed} {hoverable} {disabled} />
    ...
    {valueBind} 
    <button on:click={() => { valueBind = Math.random()*100 }}>Random Value</button>
    <button on:click={() => { valueBind = [99] }}>Array Value</button>
    <button on:click={() => { valueBind = "69" }}>Invalid Value</button>
