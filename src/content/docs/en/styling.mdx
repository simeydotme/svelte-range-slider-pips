...
The slider supports dark mode in two ways. Either by adding the `.dark` class to the slider element, 
or by the user to set `prefers-color-scheme: dark` in their browser settings.
...
1. **Manual Dark Mode**: Add the `.dark` class to the slider element:

<Demo name="Dark Mode Support" client:visible dark={true}>
  <DarkModeManual client:visible />
  <Code slot="svelte" lang="svelte" code={cleanImport(DarkModeManualRaw)} />
</Demo>

2. **Automatic Dark Mode**: The slider will automatically switch to dark mode based on system preferences:

<Demo name="Dark Mode Support" client:visible>
...
<Note title="CSS Class Name Changes" type="info">
