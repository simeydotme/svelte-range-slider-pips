...
So although [we've used `prefix` and `suffix` to add some formatting](/svelte-range-slider-pips/en/examples/prefix-suffix) before and
...
## Single Formatter
Like [in the `prefix` example](/svelte-range-slider-pips/en/examples/prefix-suffix/#simple--prefix), where we added a simple _€_ prefix. This time we will also style
