...
<Note title="Pips Placement" type="info">
  The `pipstep` property interacts with the `step` property. [Read more](/svelte-range-slider-pips/en/examples/steps-combined)
</Note>
...
<Info> Unique styling can be achieved with css, and the css selector `nth-child()`, or targeting
the `[data-val]` attribute. [See pip styling for examples](/svelte-range-slider-pips/en/styling/pips)</Info>
...
<Info>Special formatting of `labels` can be done using the `formatter()` property. [Read more about `formatter()` here](/svelte-range-slider-pips/en/examples/formatter)</Info>
