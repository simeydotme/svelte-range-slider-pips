...
import RangeFromCenter from '~demos/recipes/positive-negative/PosNegRange.svelte';
...
<Info>This can be [combined with the `rangeGapMin` property](../range-gaps/#pushy-with-gap) to 
...
<Note type="alert" title="Draggy events">
...
<Info>Here I'm using the `id` prop to give the Slider a unique identifier styling.</Info>
...
<Demo name="In-Range css style" client:visible>
  ...
  <div slot="css">
  ```css
  #sliderInRange .rangePips :is(.rsInRange, .rsSelected) {
    background: #f00;
    width: 2px;
  }
  ```
...
This is demonstrated in the [Multi-Range recipe](/svelte-range-slider-pips/en/recipes/multi-range).

<Demo name="Multi-Range" client:visible>
  <MultiRange client:visible isToggle={false} />
</Demo>
...
the `style` and `class` props with a bit of JS/CSS.

This is demonstrated in the [Positive/Negative recipe](/svelte-range-slider-pips/en/recipes/positive-negative).

<Demo name="Range from center" client:visible>
  <RangeFromCenter client:visible />
</Demo>